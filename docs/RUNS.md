...
| 3 | Dixtral on Sortformer masks, 16 meetings | Modal 16×H100 | — | **blocked on billing**: token set and verified (profile `houkinw`), AMI data (wav, cache, manual, setup) uploaded to volume `livediar-data` via `modal volume put`; app start refused with *"Please add a payment method to use H100 GPU functions"* — needs a card on the workspace (+ a $450 spend limit) | est. $12, $0 spent | — | — |
