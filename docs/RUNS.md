...
| 2 | toy conditioning ablation, 320 rooms | local (MPS) | 2026-09-12 11:47 | **v1 and v2 invalid** (recorded, not hidden): v1 word-CTC at 12.5 Hz learned nothing (dev cpWER 97.6 %); v2 char-CTC at 50 Hz on frozen Whisper-tiny features also never learned the acoustics (dev cpWER 158 %, speaker-agnostic WER 105 % after 3 k steps; decoded train windows were character soup) — a from-scratch CTC recognizer does not converge on 7 h. **v3 running** (13:5x): frozen *pretrained* wav2vec2-base-960h as the listener (verified: exact transcript on clean audio, sensible on room mixtures, identical on CPU and MPS), only the four new `<spk k>` head rows + the conditioning train (0.18 M params); arms cond 0 / 1 / 2, 1 500 steps each. Two bugs found and fixed on the way: the whole head was trainable (wrecked the pretrained rows at warm-up) and the additive conditioning had unit scale at init and swamped the encoder input — now zero-gated, which is the right design for any pretrained host and is carried into PLAN §2 | $0 | pending | yes: conditioning must be zero-gated at init when injected into a pretrained model |
| 3 | Dixtral on Sortformer masks, 16 meetings | Modal 16×H100 | 2026-09-12 16:5x | **running** (`modal run --detach ...::dixtral_ami`, app `livediar-research`); payment method added by the user; every job now passes a budget guard (`LIVEDIAR_BUDGET_USD`, default $450) against `spend.jsonl` before starting | est. $12 | pending | — |
...
| 4 | scaled ablation v3, 2 000 rooms / 3 k steps, arms cond 0/1/2 | Modal A100 (+ local 320-room run) | 2026-09-12 17:0x | launching: 600 MB LibriSpeech pool uploaded, rooms simulated in-container, then three arms on the A100 | est. $6 | pending | — |
